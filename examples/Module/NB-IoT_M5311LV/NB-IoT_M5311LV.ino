    ...
    if (Serial2.available() != 0) {
      ...
      if (restr.indexOf("\r\n") != -1) {
        if (restr.indexOf("+IP:") != -1) {
          String ipstr =
              restr.substring(restr.indexOf("+IP:") + String("+IP:").length(),
                              restr.indexOf("\r\n", restr.indexOf("+IP:")));
        ...
        if (restr.indexOf("transmitted") != -1) {
          String substr = restr.substring(
              restr.indexOf("transmitted") + String("transmitted,").length(),
...
void setup() {
  ...
  for (int i = 0; i < 3;
       i++)  //Pin 32 pull up 2 seconds on (3 seconds for safety), 8 seconds off
  {
