...
void powerSetup() {
  ...
  DisPowerbuff.drawColorBitmap(0, 39, 77, 24, (uint8_t *)image_power, 0xff9c00,
                               0x000000);
...
void choosePower() {
  ...
  DisPowerbuff.drawColorBitmap(0, 0, 77, 37, (uint8_t *)image_Internal, color1,
                               0x000000);

  DisPowerbuff.drawColorBitmap(0, 65, 77, 37, (uint8_t *)image_External, color2,
                               0x000000);
...
void MPU6886Test() {
  ...
  for (int n = 0; n < 12; n++) {
    line3d.RotatePoint(&rect_source[n].start_point, &rect_dis.start_point,
                       theta, phi, (double)0);
    line3d.RotatePoint(&rect_source[n].end_point, &rect_dis.end_point, theta,
                       phi, (double)0);
...
void clockFlush() {
  ...
  DisClockbuff.drawColorBitmap(posx[0], 26, 24, 42,
                               DigNumber[sytState.Rtctime.Hours / 10], 0xff9c00,
                               0x1a1100);
  ...
  DisClockbuff.drawColorBitmap(0, 0, 164, 17, rectptrBuff[sytState.clockCount],
                               0xff9c00, 0x000000);
...
void batpowerFlush() {
  ...
  DisBatbuff.drawColorBitmap(3, 37, 60, 10, Number_7x10px[14], color, 0x000000);
  ...
  DisBatbuff.drawColorBitmap(3, 20, 7, 10, Number_7x10px[batVoltage1], 0xff9c00,
                             0x000000);
...
void sdcardSetup() {
  ...
  if (Type == CARD_UNKNOWN || Type == CARD_NONE) {
    ...
    DisSDCardbuff.drawColorBitmap(
        ...
        0xff9c00, 0x000000);
    DisSDCardbuff.drawColorBitmap(
        40, 19, 7, 10, (uint8_t *)Number_7x10px[sdcardSize / 10 % 10], 0xff9c00,
        0x000000);
    ...
    if (WifiNumber == -2) {
      ...
      if (first) {
        ...
        if (count >= 10) {
          count = 0;
          Disbuff.fillRect(0, 34, 320, 173, TFT_BLACK);
          Disbuff.drawColorBitmap(122, 98, 76, 44, TouchFishBuff[count_n],
                                  0xff9c00, 0x000000);
          Disbuff.pushSprite(0, 0);
          count_n++;
...
void AppTimer() {
  ...
  while (1) {
    ...
    Timerbuff.drawColorBitmap(posx[0], 4, 24, 42, DigNumber[min / 10], colorNum,
                              0x000000);
    Timerbuff.drawColorBitmap(posx[1], 4, 24, 42, DigNumber[min % 10], colorNum,
                              0x000000);
    Timerbuff.drawColorBitmap(posx[2], 4, 24, 42, DigNumber[sec / 10], colorNum,
                              0x000000);
    Timerbuff.drawColorBitmap(posx[3], 4, 24, 42, DigNumber[sec % 10], colorNum,
                              0x000000);
    Timerbuff.drawColorBitmap(posx[4], 14, 18, 35, DigNumber_35px[mill / 10],
                              colorNum, 0x000000);
    Timerbuff.drawColorBitmap(posx[5], 14, 18, 35, DigNumber_35px[mill % 10],
                              colorNum, 0x000000);
