...
void setup() {
  ...
  WiFi.mode(
      WIFI_AP_STA);  // Set the wifi mode to the mode compatible with the AP and Station, and start intelligent network configuration
  ...
  while (
      !WiFi.smartConfigDone()) {  //If the smart network is not completed.  若智能配网没有完成
    delay(500);
    M5.Lcd.print(".");
  }
  ...
  while (
      WiFi.status() !=
      WL_CONNECTED) {  //M5Core2 will connect automatically upon receipt of the configuration information, and return true if the connection is successful.  收到配网信息后M5Core2将自动连接，若连接成功将返回true
    delay(500);
    M5.Lcd.print(".");
  }
  M5.Lcd.print("\nWiFi Connect To: ");
