...
const char* apSSID =
    "M5STACK_SETUP";  //Define the name of the created hotspot.  定义创建热点的名称
boolean settingMode;
String ssidList;
String wifi_ssid;  //Store the name of the wireless network.  存储无线网络的名称
String
    wifi_password;  //Store the password of the wireless network.  存储无线网络的密码
...
void setup() {
  ...
  settingMode =
      true;  //If there is no stored wifi configuration information, turn on the setting mode.  若没有已存储的wifi配置信息,则开启设置模式
  setupMode();
}
...
void loop() {
  ...
  webServer
      .handleClient();  //Check that there is no facility to send requests to the M5StickC Web server over the network.  检查有没有设备通过网络向M5Core2网络服务器发送请求
}

boolean
restoreConfig() { /* Check whether there is wifi configuration information storage, if there is 1 return, if no return 0.
...
boolean checkConnection() {  //Check wifi connection.  检测wifi连接情况
  ...
  while (
      count <
      30) { /* If you fail to connect to wifi within 30*350ms (10.5s), return false; otherwise return true.
...
void startWebServer() {  //Open the web service.  打开Web服务
  if (settingMode) {  //If the setting mode is on.  如果设置模式处于开启状态
    ...
    M5.Lcd.print(
        WiFi.softAPIP());  //Output AP address (you can change the address you want through apIP at the beginning).  输出AP地址(可通过开头的apIP更改自己想要的地址)
    webServer.on(
    ...
    webServer.on("/", []() {  //AP web interface settings.  AP网页界面设置
      String s =
          "<h1>STA mode</h1><p><a href=\"/reset\">Reset Wi-Fi Settings</a></p>";
...
void setupMode() {
  WiFi.mode(
      WIFI_MODE_STA);  //Set Wi-Fi mode to WIFI_MODE_STA.  设置Wi-Fi模式为WIFI_MODE_STA
  WiFi.disconnect();  //Disconnect wifi connection.  断开wifi连接
  delay(100);
  int n =
      WiFi.scanNetworks();  //Store the number of wifi scanned into n.  将扫描到的wifi个数存储到n中
  delay(100);
  M5.Lcd.println("");
  for (
      int i = 0; i < n;
      ++i) {  //Save each wifi name scanned to ssidList.  将扫描到的每个wifi名称保存到ssidList中
