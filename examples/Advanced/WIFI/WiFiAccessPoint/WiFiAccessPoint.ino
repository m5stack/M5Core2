...
void setup() {
  ...
  WiFi.softAP(
      ssid,
      password);  // You can remove the password parameter if you want the AP to be open.  如果你想建立开放式热点,可以删除密码
  IPAddress myIP =
      WiFi.softAPIP();  //Get the softAP interface IP address.  获取AP接口IP地址
  M5.lcd.println(myIP);
  server
      .begin();  //Start the established Internet of Things network server.  启动建立的物联网网络服务器
}

void loop() {
  WiFiClient client =
      server
          .available();  // listen for incoming clients.  检查有没有设备通过网络向M5Stack网络服务器发送请求

  if (client) {  // if you get a client.  如果收到请求
    M5.lcd.print("New Client:");
    String currentLine =
        "";  // make a String to hold incoming data from the client.  创建一个String来保存来自客户端的传入数据
    while (
        client
            .connected()) {  // loop while the client's connected,continuously receiving data.  在客户端连接时进行循环,不断接收数据
      if (client
              .available()) {  // if there's bytes to read from the client.  如果有数据可读取
        ...
        if (c ==
          ...
          if (currentLine.length() == 0) {
            ...
            client.print(
                "Click <a href=\"/High\">here</a> to turn ON the LED.<br>");
            client.print(
                "Click <a href=\"/Low\">here</a> to turn OFF the LED.<br>");
            ...
            c !=
            '\r') {  // if you got anything else but a carriage return character.  如果你得到了除了回车符以外的其他字符，
          currentLine +=
              c;  // add it to the end of the currentLine.  将它添加到currentLine的末尾
        }
