...
*                          配套  M5Core2 示例源代码
* Visit for more information: https://docs.m5stack.com/en/unit/color
* 获取更多资料请访问: https://docs.m5stack.com/zh_CN/unit/color
*
...
#define commonAnode \
  true  //set to false if using a common cathode LED.  //如果使用普通阴极LED，则设置为false
...
void setup() {
  ...
  tcs.setIntegrationTime(
      TCS34725_INTEGRATIONTIME_154MS);  //Sets the integration time for the TC34725.  设置TC34725的集成时间
  tcs.setGain(
      TCS34725_GAIN_4X);  //Adjusts the gain on the TCS34725.  调整TCS34725上的增益
}
...
void loop() {
  ...
  tcs.getRawData(
      &red, &green, &blue,
      &clear);  //Reads the raw red, green, blue and clear channel values.  读取原始的红、绿、蓝和清晰的通道值
