...
void setup() {
  M5.begin(
      true, false, true,
      true);  //Init M5Core2 And the I2C port(21,22).  初始化 M5Core2和I2C端口(21,22)
}

void loop() {
  while (
      !qmp6988
           .init()) {  //Init qmp6988.True if the init was successful, otherwise false.   初始化qmp6988,如果初始化成功返回1
  ...
  M5.lcd.fillScreen(
      BLACK);  //Fill the screen with black (to clear the screen).  将屏幕填充满黑色(用来清屏)
  M5.Lcd.printf(
      "Pressure:%0.2f",
      qmp6988
          .calcPressure());  //Screen print format string.  屏幕打印格式化字符串
