 ...
 * The interface is Productd in the datasheet section 8.1.2.
 */
...
byte MFRC522::PICC_Select(
  ...
  byte index;
  byte
      uidIndex;  // The first index in uid->uidByte[] that is used in the current Cascade Level.
  int8_t
      currentLevelKnownBits;  // The number of known UID bits in the current Cascade Level.
  byte buffer
      [9];  // The SELECT/ANTICOLLISION commands uses a 7 byte standard frame + 2 bytes CRC_A
  byte
      bufferUsed;  // The number of bytes used in the buffer, ie the number of bytes to transfer to the FIFO.
  byte
      rxAlign;  // Used in BitFramingReg. Defines the bit position for the first bit received.
  byte
 ...
 * The authentication is Productd in the MFRC522 datasheet section 10.3.1.9 and http://www.nxp.com/documents/data_sheet/MF1S503x.pdf section 10.1.
...
byte MFRC522::MIFARE_Restore(byte blockAddr  ///< The block (0-0xff) number.
  ...
  // The datasheet Products Restore as a two step operation, but does not explain what data to transfer in step 2.
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(
  ...
  byte status;
  byte
      firstBlock;  // Address of lowest address to dump actually last block dumped)
  byte no_of_blocks;  // Number of blocks in sector
  bool
      isSectorTrailer;  // Set to true while handling the "last" (ie highest address) in the sector.
