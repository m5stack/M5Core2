...
Point and Zone: Describing Points and Areas on the Screen
    ...
    The Point and Zone classes allow you to create variables that hold a
    point or an area on the screen. You can
    ...
    'contains' method you can test if a point is in a zone. There is also
    a 'distanceTo' method to see how far two points are apart. Here are some
...
Basic Touch API
    ...
    The basic API provides a way to read the data from the touch sensor.
    ...
    M5.update()
        In the loop() part of your sketch, call "M5.update()". This will in
        turn call M5.Touch.update(), which is the only part that talks to
        the touch interface. It updates the data used by the rest of the
        API.
    ...
    M5.Touch.point[0], M5.Touch.point[1]
        M5.Touch.point[0] and M5.Touch.point[1] are Points that hold the
        detected touches.
...
Buttons, Gestures, Events
    ...
    Note tha you may not want to use any of the above directly. The
    M5Buttons library provides Button, Gesture and Events objects that
    allow you to quickly create reactive visual buttons on the screen and
    react differently based on whether a button was clicked, tapped, or
    double-tapped. Have a look at the documentation for that, which is in
    the M5Button.h file in the src/utility directory of this repository.
    The example under File / Examples / M5Core2 / Basics / touch in your
    Arduino environment should give you an idea of what's possible.
...
Screen Rotation
    ...
    * What that means is that either x or y for the area below the screen
      may go negative. Say you use the screen upside-down with
      M5.Lcd.setRotation(3). In that case the off-screen touch area (Y
      coordinates 240 through 279) that was below the screen now becomes
      above the screen and has Y coordinates -40 through -1.

    * See the button library for a feature that allows you to keep some
      Zone and Button objects in the same place on the physical screen,
      regardless of rotation.

    M5.Touch.rotation
        Holds the current rotation, in the same format as the rotation for
        the display. Is normally set by the M5Display when it receives
        M5.Lcd.setRotation, but can be changed manually also.



TFT_eSPI Resistive Touch API emulation

    While technically not part of this library itself, we added an
    emulation for the TFT_eSPI touch API to the M5Display object that
    merely passes informaton on to the M5.Touch object. So M5.Lcd can be
    addressed as if it's a touch screen using that older resistive touch
    interface. Do note that this interface is not nearly as powerful as
    M5.Touch's native API. But together with M5Button's TFT_eSPI_Button
    emulation, this should allow you to compile lots of ESP32 software
    written for touch screens.
...
Advanced Uses of the Touch Library
    ...
    M5.Touch.wasRead
        True if the sensor was actually read. The sensor can only provide
        updates every 13 milliseconds or so. M5.update() can loop as quick
        as once every 20 MICROseconds, meaning it would continually read
        the sensor when there was nothing to read. So M5.Touch.read() only
        really reads when it's time to do so, and returns with
        M5.Touch.wasRead false otherwise.

    M5.point0finger
        The FT6336 chip keeps track of fingers, each touch has a finger
        ID of 0 or 1. So when there are two touches in point[0] and
        point[1] and then one goes away, point0finger allows you to see
        which touch is left in point[0].

    M5.Touch.interval()
        Without arguments returns the current interval between sensor
        updates in milliseconds. If you supply a number that's the new
        interval. The default of 13 seems to give the most updates per
        second.

    M5.Touch.ft6336(reg)
    M5.Touch.ft6336(reg, value)
    M5.Touch.ft6336(reg, size, *data)
        Allows you to read and write registers on the ft6336 touch
        interface chip directly. The first form reads one byte, the second
        form writes one and the third form reads a block of 'size' bytes
        starting at 'reg' into a buffer at '*data'.

    M5.Touch.dump()
        M5.Touch.dump() dumps the entire register space on the FT6336 chip
        as a formatted hexdump to the serial port.
...
Legacy API
    ...
    little bit like Buttons. This older API is still supported (the
...
Example
    ...
    library and M5Button in action. Please have a look at it to understand
    how this all works and run the sketch to see all the events printed to
    the serial port. It shows buttons, gestures and events and should be
