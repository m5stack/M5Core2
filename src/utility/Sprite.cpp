...
uint16_t TFT_eSprite::readPixel(int32_t x, int32_t y) {
  ...
  if (_bpp == 8) {
    ...
    if (color != 0) {
      ...
      color = (color & 0xE0) << 8 | (color & 0xC0) << 5 | (color & 0x1C) << 6 |
...
void TFT_eSprite::drawGlyph(uint16_t code) {
  ...
  if (found) {
    ...
    for (int32_t y = 0; y < this->gHeight[gNum]; y++) {
        ...
        if (pixel) {
          if (pixel != 0xFF) {
            if (dl) {
              drawFastHLine(
                  xs,
                  y + this->cursor_y + this->gFont.maxAscent - this->gdY[gNum],
                  dl, fg);
              dl = 0;
            }
            if (_bpp != 1)
              drawPixel(
                  x + this->cursor_x + this->gdX[gNum],
                  y + this->cursor_y + this->gFont.maxAscent - this->gdY[gNum],
      ...
      if (dl) {
        drawFastHLine(
            xs, y + this->cursor_y + this->gFont.maxAscent - this->gdY[gNum],
            dl, fg);
        dl = 0;
      }
    }
